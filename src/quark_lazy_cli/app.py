...
""")
            ...
            if fresh_invalid_count_display > 0:
                fresh_label = f"🔍 {fresh_days} 天内新鲜资源：搜索到 {fresh_total} 个，验证资源有效后：{len(fresh)} 个（其中 {fresh_invalid_count_display} 个失效）"
            else:
                fresh_label = f"🔍 {fresh_days} 天内新鲜资源：搜索到 {fresh_total} 个，验证资源有效后：{len(fresh)} 个"
            print(fresh_label)
            print("=" * 60)
            ...
            if old_combined:
                ...
                if old_invalid_count_display > 0:
                    old_label = f"🔍 {fresh_days}-{old_days} 天旧资源：搜索到 {old_total} 个，验证资源有效后：{len(old)} 个（其中 {old_invalid_count_display} 个失效）"
                else:
                    old_label = f"🔍 {fresh_days}-{old_days} 天旧资源：搜索到 {old_total} 个，验证资源有效后：{len(old)} 个"
                print(old_label)
                print("=" * 60)
    ...
    def _compute_attainable_target_eps(self, ctx) -> set:
        # attainable_to_save_eps_set: 可下载目标剧集
        # 只做 target_save_eps_set ∩ pool_eps
        # 不判断 update_mode，不计算 planned/new_base/holes
        all_pool_eps = set()
        for entry in ctx.bfs_result_pool:
            all_pool_eps |= entry["eps"]
        ctx.attainable_to_save_eps_set = ctx.target_save_eps_set & all_pool_eps
    ...
    def _bfs_build_resource_pool(self, ctx) -> list:
        ...
        for i, current_url in enumerate(ctx.focused_urls_list):
            if settings.debug:
                sys.stderr.write(f"\n{'='*60}\n")
                sys.stderr.write(f"[_bfs_build_resource_pool] BFS URL] Processing URL {i}: {current_url}\n")
            res = self._get_share_detail_with_qas_pattern(ctx, current_url)
            if settings.debug:
                sys.stderr.write(f"[_bfs_build_resource_pool] get_share_detail success={res.get('success')} url={current_url}\n")
                if not res.get("success"):
                    sys.stderr.write(f"[_bfs_build_resource_pool] get_share_detail failed: {res}\n")
                else:
                    sys.stderr.write(f"[_bfs_build_resource_pool] root list len={len(res.get('data', {}).get('list', []))}\n")
            if not res.get("success"):
                continue
            if settings.debug:
                sys.stderr.write(f"\n[_bfs_build_resource_pool] [BFS URL] {current_url}\n")
    ...
    def _build_dual_candidate_pools(self, ctx) -> None:
        ...
        if settings.debug:
            sys.stderr.write(
                f"\n\n[_build_dual_candidate_pools] 构建双候选池: "
                f"可下载目标剧集={sorted(ctx.attainable_to_save_eps_set)} "
                f"候选池中可下载链接数量={len(ctx.bfs_result_pool)}\n"
            )
        ...
        if settings.debug:
            ...
            sys.stderr.write(f"[_build_dual_candidate_pools] 按候选下载链接展示剧集PK结果:\n")
            for i, entry in enumerate(ctx.bfs_result_pool):
                url_short = entry["url"][-40:]
                b_eps = sorted(batch_by_url.get(i, []))
                o_eps = sorted(onebyone_by_url.get(i, []))
                sys.stderr.write(
                    f"  候选下载链接[{i}] url={url_short} | 批量下载候选剧集 regex_or_exact_batch_save_candidates={b_eps} | 逐一转存候选剧集 exact_one_by_one_candidates={o_eps}\n"
                )
            sys.stderr.write(f"[_build_dual_candidate_pools] 转存队列汇总:\n")
            sys.stderr.write(
                f"  批量下载候选剧集 regex_or_exact_batch_save_candidates={sorted(regex_or_exact_batch_save_candidates.keys())}\n"
            )
            sys.stderr.write(
                f"  逐一转存候选剧集 exact_one_by_one_candidates={sorted([item['ep'] for item in exact_one_by_one_candidates])}\n"
            )
    ...
    def _execute_phase1_phase2_quark_auto_save(self, ctx) -> set:
        ...
        for cand_url, ep_cand_list in batch_by_url.items():
            ...
            if settings.debug:
                sys.stderr.write(
                    f"\n[_execute_phase1_phase2_quark_auto_save] [Phase1-批量转存模式]\n"
                    f"  候选URL节点完整路径: {cand_url}\n"
                    f"  批量EP: {sorted(batch_eps)}\n"
                    f"  文件名匹配规则: {smart_pattern}\n"
                    f"  输出命名模板: {target_task_run['replace']}\n\n"
                )
        ...
        for item in ctx.exact_one_by_one_candidates:
            ...
            if settings.debug:
                ...
                best_cand_fn = fname
                sys.stderr.write(f"[_execute_phase1_phase2_quark_auto_save] [Phase2-逐一转存模式] ep={ep} pool_entry_url={pool_entry['url']} best_cand_url={best_cand_url} best_cand_fn={best_cand_fn}\n")
            # ========== DEBUG END ==========
